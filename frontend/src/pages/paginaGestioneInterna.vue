      ...
      <section class="mx-auto flex w-full max-w-6xl flex-col items-center gap-8">
        ...
        <div v-else class="w-full space-y-8">
          ...
          <div v-else-if="contrattoAttivo && vistaAttiva === 'info'" class="w-full rounded-2xl border border-zinc-200 bg-white shadow-lg">
            <div class="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-stretch lg:justify-between">
              <!-- Sezione sinistra: dati appartamento -->
              <div class="flex-1">
              ...
              <div class="flex flex-col justify-center gap-3 lg:w-56">
                <button
                  type="button"
    ...
    <transition name="fade">
      <div v-if="showListaSpesa" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative mx-4 w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl sm:p-7">
                ...
                <div class="flex items-center gap-4">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <input
                      ...
                      min="1"
                      :class="item.preso
                        ? 'line-through w-8 border-0 bg-transparent p-0 text-sm font-semibold text-zinc-400 outline-none focus:ring-0'
                        : 'w-8 border-0 bg-transparent p-0 text-sm font-semibold text-zinc-800 outline-none focus:ring-0'"
                      :value="item.quantita"
          ...
          <div v-if="listaSpesaError" class="mt-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            {{ listaSpesaError }}
          </div>
...
function mostraGiorniRifiuti(giorni) {
  if (!Array.isArray(giorni) || giorni.length === 0) return 'Nessun giorno impostato'
...
async function salvaCalendarioRifiuti() {
  ...
  if (response.success) {
    ...
    setTimeout(() => { calendarioRifiutiSuccess.value = '' }, 3000)
  } else {
    calendarioRifiutiError.value = response.error
  }
...
function getPriorityColor(priorita) {
  ...
  switch (p) {
    case 'scarsa': return 'text-yellow-700 font-semibold'
    case 'media': return 'text-orange-600 font-semibold'
    case 'urgente': return 'text-red-700 font-semibold'
    default: return 'text-zinc-900 font-semibold'
  }
}
...
function toggleFiltro(tipo) {
  if (tipo === 'turni') filtroTurniAttivo.value = !filtroTurniAttivo.value
  else if (tipo === 'rifiuti') filtroRifiutiAttivo.value = !filtroRifiutiAttivo.value
}
...
watch(vistaAttiva, (value) => {
  if (value === 'calendario') mostraGuasti.value = false
})
