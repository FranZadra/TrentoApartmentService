...
describe('Suite di test gestione interna', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });

    describe('POST /api/v2/gestione-interna/guasti', () => {
        // TEST CASE N.66
        test('Inserimento segnalazione guasto bloccato se manca la priorità', async () => {
            ...
            .post('/api/v2/gestione-interna/guasti')
    ...
    describe('PUT /api/v2/gestione-interna/spesa/:contrattoId', () => {
        // TEST CASE N.80
        test('Aggiornamento lista della spesa fallisce se un articolo ha quantità <= 0', async () => {
            ...
            const res = await request(app)
            .put('/api/v2/gestione-interna/spesa/64e000000000000000000005')
            .set('Authorization', `Bearer ${tokenInquilino}`)
            .send({
    ...
    describe('POST /api/v2/gestione-interna/faccende/:appId', () => {
        // TEST CASE N.83
        test('Inserimento con successo di una nuova faccenda condivisa', async () => {
            ...
            const res = await request(app)
            .post('/api/v2/gestione-interna/faccende/64e000000000000000000005')
            .set('Authorization', `Bearer ${tokenInquilino}`)
            .send({
        ...
        test('Tentativo eliminazione faccenda altrui restituisce 403', async () => {
            ...
            const res = await request(app)
            .delete(`/api/v2/gestione-interna/faccende/64e000000000000000000005/${faccendaDiUnAltro._id}`)
            .set('Authorization', `Bearer ${tokenInquilino}`);
